...
class _MyVideoScreenState extends State<MyVideoScreen> {
  late List<VideoData> videoDataList;
          ...
          children: [
            for (int i = 0; i < videoDataList.length; i++)
              Column(
                ...
                children: [
                  YoutubePlayer(
                    controller: videoDataList[i].controller,
                              ...
                              MaterialPageRoute(
                                builder: (context) => VideoDetailPage(
                                  videodata: videoDataList[
                                      i], // parametre gönderilmeli
                                ),
                              ),
                            );
                          },
                          child: Text(videoDataList[i].buttonText),
                        ),
                      ),
                    ],
                  ),
                  SizedBox(height: 8.0),
                  if (videoDataList[i].isTextVisible)
                    Padding(
                      padding: const EdgeInsets.all(8.0),
                      child: Text(
                        videoDataList[i].text,
